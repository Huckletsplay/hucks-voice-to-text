...
//!
//! `#[ignore]`d: it needs the network and downloads the full release (~133 MB). Run on purpose after
...
fn update_live_the_published_release_is_found_downloaded_and_verified() {
    ...
    let file = update::download(&offer).expect("the download arrives and matches its checksum");
    assert_eq!(std::fs::metadata(&file).unwrap().len(), offer.download_size);
